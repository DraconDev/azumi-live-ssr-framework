...
**v1 Limitations**: The expression evaluator supports field lookup (`active_tab`), equality (`== 'rust'`), inequality (`!= 'val'`), negation (`!is_open`), truthy checks, ternary expressions (`field ? 'a' : 'b'`), and compound operators (`&&`, `||`).
