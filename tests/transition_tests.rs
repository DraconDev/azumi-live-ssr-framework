...
fn test_transition_fade_attribute_renders() {
    ...
    fn fade_box() -> impl Component {
        html! {
            <div az-transition:fade>"Fade content"</div>
        }
    }
...
fn test_transition_slide_attribute_renders() {
    ...
    fn slide_box() -> impl Component {
        html! {
            <div az-transition:slide>"Slide content"</div>
        }
    }
...
fn test_transition_scale_attribute_renders() {
    ...
    fn scale_box() -> impl Component {
        html! {
            <div az-transition:scale>"Scale content"</div>
        }
    }
...
fn test_multiple_transitions_in_same_component() {
    ...
    fn multi_transition() -> impl Component {
        html! {
            <div>
                <p az-transition:fade>"Fade paragraph"</p>
                <p az-transition:slide>"Slide paragraph"</p>
                <p az-transition:scale>"Scale paragraph"</p>
            </div>
        }
    }
...
fn test_transition_with_keyed_list() {
    ...
    assert!(output.contains("az-transition:fade"), "Transition + @keyed should work. Got: {}", output);
...
fn test_transition_on_nested_elements() {
    ...
    fn nested_transitions() -> impl Component {
        html! {
            <div az-transition:fade>
                <span az-transition:slide>"Nested slide"</span>
            </div>
        }
    }
